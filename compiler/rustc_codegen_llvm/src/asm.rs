            ...
            match *op {
                InlineAsmOperandRef::Out { reg, late, place } => {
                        ...
                        || reg.reg_class().is_clobber_only(asm_arch)
                    {
                        ...
                        clobbers.push(format!("~{}", reg_to_llvm(reg, None)).into());
                        continue;
                    } else {
                    ...
                    constraints.push(format!("{}{}", prefix, reg_to_llvm(reg, layout)).into());
                }
                InlineAsmOperandRef::InOut { reg, late, in_value, out_place } => {
                    ...
                    let prefix = if late { "=" } else { "=&" };
                    constraints
                        .push(format!("{}{}", prefix, reg_to_llvm(reg, Some(layout))).into());
                }
                _ => {}
            }
        }
